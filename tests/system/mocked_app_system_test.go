...
func TestDeployApp(t *testing.T) {
	...
	deployReq := &common.Request{
		...
		RequestID:     RequestID,
		Payload:       uploadArtifactAndBuildDescriptorPayload(t, suite, ApplicationId, []byte("deploy-payload")),
...
func TestMockRuntimeFullFlow(t *testing.T) {
	...
	deployReq := &common.Request{
		...
		Payload:       uploadArtifactAndBuildDescriptorPayload(t, suite, appID, []byte("mock-runtime-app-bytecode")),
		Sender:        suite.GetManagerAllowedDeployer(),
		Timestamp:     common.ToBig(new(big.Int).SetInt64(time.Now().Unix())),
		DepositAmount: common.NewBig(0),
		MaxFeeValue:   common.NewBig(100),
