...
func TestSimpleAppIntegration(t *testing.T) {
	...
	depositState1Bytes, depositEvents, depositAppEvents, fuel, failure := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), deposit1Amount, initialStateBytes, wasmBytes)
	...
	depositState2Bytes, depositEvents, _, fuel, failure := runtime.Deposit(ctx, appId, ethCommon.Address(user2Address), velacommon.NativeTokenAddress(), deposit2Amount, depositState1Bytes, wasmBytes)
	require.Nil(t, failure)
...
func TestSimpleAppIntegration_NegativeScenarios(t *testing.T) {
	...
	// User1 deposits 1000
	populatedStateBytes, _, _, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), big.NewInt(1000), initialStateBytes, wasmBytes)
	require.Nil(t, err)
	// User2 deposits 500
	populatedStateBytes, _, _, _, err = runtime.Deposit(ctx, appId, ethCommon.Address(user2Address), velacommon.NativeTokenAddress(), big.NewInt(500), populatedStateBytes, wasmBytes)
	require.Nil(t, err)
...
func TestSimpleAppIntegration_NilData(t *testing.T) {
	...
	t.Run("deposit with nil state", func(t *testing.T) {
		...
		_, _, _, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), big.NewInt(1000), nil, wasmBytes)
		require.Error(t, err)
...
func TestSimpleAppIntegration_InvalidWasm(t *testing.T) {
	...
	t.Run("deposit with nil wasm", func(t *testing.T) {
		_, _, _, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), big.NewInt(1000), initialStateBytes, nil)
		require.Error(t, err)
...
func TestSimpleAppIntegration_InvalidState(t *testing.T) {
	...
	t.Run("deposit with invalid state", func(t *testing.T) {
		_, _, _, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), big.NewInt(1000), invalidState, wasmBytes)
		require.Error(t, err)
		require.Contains(t, err.Error(), "Failed to parse application state")
	})
	...
	for i := range numGoroutines {
		go func(goroutineIndex int) {
			...
			for j := 0; j < iterationsPerGoroutine; j++ {
				...
				runtimeMutex.Lock()
				newStateBytes, _, _, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(userAddress), velacommon.NativeTokenAddress(), depositAmount, stateBytes, wasmBytes)
...
func TestSimpleAppIntegration_MemoryCleanBetweenOps(t *testing.T) {
	...
	stateBytes, _, _, _, failure := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), big.NewInt(5000), stateBytes, wasmBytes)
	require.Nil(t, failure)
	...
	stateBytes, _, _, _, failure = runtime.Deposit(ctx, appId, ethCommon.Address(user2Address), velacommon.NativeTokenAddress(), big.NewInt(3000), stateBytes, wasmBytes)
	require.Nil(t, failure)
...
func TestSimpleAppIntegration_ErrorPathMemory(t *testing.T) {
	...
	stateBytes, _, _, _, failure := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), big.NewInt(100), stateBytes, wasmBytes)
	require.Nil(t, failure)
	...
	_, _, _, _, failure = runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), big.NewInt(100), []byte("{bad-json}"), wasmBytes)
...
func TestSimpleAppIntegration_LargeResultRoundTrip(t *testing.T) {
	...
	for i := range numAccounts {
		...
		newState, _, _, _, failure := runtime.Deposit(ctx, appId, ethCommon.Address(addr), velacommon.NativeTokenAddress(), big.NewInt(int64(1000+i)), stateBytes, wasmBytes)
...
func TestMixedETHAndERC20Deposits(t *testing.T) {
	...
	stateBytes, _, _, _, failure := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), velacommon.NativeTokenAddress(), ethAmount, stateBytes, wasmBytes)
	require.Nil(t, failure)
