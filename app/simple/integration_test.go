...
func TestSimpleAppIntegration(t *testing.T) {
	...
	depositState1Bytes, depositEvents, failure := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), deposit1Amount, initialStateBytes, wasmBytes)
	...
	depositState2Bytes, depositEvents, failure := runtime.Deposit(ctx, appId, ethCommon.Address(user2Address), deposit2Amount, depositState1Bytes, wasmBytes)
	require.Nil(t, failure)
	...
	withdrawStateBytes, withdrawEvents, withdrawals, err := runtime.ProcessRequest(ctx, appId, ethCommon.Address(user1Address), payloadBytes, depositState2Bytes, wasmBytes)
	require.Nil(t, err)
	...
	compareStateBytes, events, withdrawals, err := runtime.ProcessRequest(ctx, appId, ethCommon.Address(user1Address), payloadBytes, withdrawStateBytes, wasmBytes)
	require.Nil(t, err)
...
func TestSimpleAppIntegration_NullPayload(t *testing.T) {
	...
	t.Run("null payload json", func(t *testing.T) {
		nullPayload := []byte{}
		_, _, _, err := runtime.ProcessRequest(ctx, appId, ethCommon.Address(user1Address), nullPayload, initialStateBytes, wasmBytes)
		require.Nil(t, err)
	})
}
...
func TestSimpleAppIntegration_NegativeScenarios(t *testing.T) {
	...
	// User1 deposits 1000
	populatedStateBytes, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), big.NewInt(1000), initialStateBytes, wasmBytes)
	require.Nil(t, err)
	// User2 deposits 500
	populatedStateBytes, _, err = runtime.Deposit(ctx, appId, ethCommon.Address(user2Address), big.NewInt(500), populatedStateBytes, wasmBytes)
	require.Nil(t, err)
	...
	t.Run("withdraw with insufficient balance", func(t *testing.T) {
		...
		_, _, _, err = runtime.ProcessRequest(ctx, appId, ethCommon.Address(user1Address), payloadBytes, populatedStateBytes, wasmBytes)
		require.Error(t, err)
	...
	t.Run("withdraw from non-existent account", func(t *testing.T) {
		...
		_, _, _, err = runtime.ProcessRequest(ctx, appId, ethCommon.Address(nonExistentUser), payloadBytes, populatedStateBytes, wasmBytes)
		require.Error(t, err)
	...
	t.Run("unsupported instruction type", func(t *testing.T) {
		...
		_, _, _, err := runtime.ProcessRequest(ctx, appId, ethCommon.Address(user1Address), []byte(payload), populatedStateBytes, wasmBytes)
		require.Error(t, err)
...
func TestSimpleAppIntegration_NilData(t *testing.T) {
	...
	t.Run("deposit with nil state", func(t *testing.T) {
		...
		_, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), big.NewInt(1000), nil, wasmBytes)
		require.Error(t, err)
...
func TestSimpleAppIntegration_InvalidWasm(t *testing.T) {
	...
	t.Run("deposit with nil wasm", func(t *testing.T) {
		_, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), big.NewInt(1000), initialStateBytes, nil)
		require.Error(t, err)
	...
	t.Run("process request with nil wasm", func(t *testing.T) {
		...
		_, _, _, err := runtime.ProcessRequest(ctx, appId, ethCommon.Address(user1Address), payload, initialStateBytes, nil)
		require.Error(t, err)
		require.Contains(t, err.Error(), "failed to load module")
	})
...
func TestSimpleAppIntegration_InvalidState(t *testing.T) {
	...
	t.Run("deposit with invalid state", func(t *testing.T) {
		_, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(user1Address), big.NewInt(1000), invalidState, wasmBytes)
		require.Error(t, err)
		require.Contains(t, err.Error(), "Failed to parse application state")
	})
	...
	t.Run("process request with invalid state", func(t *testing.T) {
		payload := []byte(`{"type":"withdraw","withdraw":{"to":"some_address","amount":100}}`)
		_, _, _, err := runtime.ProcessRequest(ctx, appId, ethCommon.Address(user1Address), payload, invalidState, wasmBytes)
		require.Error(t, err)
		require.Contains(t, err.Error(), "Failed to parse application state")
	})
	...
	for i := 0; i < numGoroutines; i++ {
		go func(goroutineIndex int) {
			...
			for j := 0; j < iterationsPerGoroutine; j++ {
				...
				runtimeMutex.Lock()
				newStateBytes, _, err := runtime.Deposit(ctx, appId, ethCommon.Address(userAddress), depositAmount, stateBytes, wasmBytes)
				...
				runtimeMutex.Lock()
				processStateBytes, _, _, err := runtime.ProcessRequest(ctx, appId, ethCommon.Address(userAddress), withdrawPayloadBytes, stateBytes, wasmBytes)
