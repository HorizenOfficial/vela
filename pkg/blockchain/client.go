...
func (c *BlockChainClient) MarkRequestFailed(ctx context.Context, requestID common.RequestIdType) error {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	c.account.Value = nil
	return c.sendTxAndWaitMined(ctx, c.processorEndpoint.PackMarkRequestFailed(requestID))
