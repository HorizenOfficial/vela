...
- The Executor binary is built from `dockerfiles/executor/Dockerfile` (not yet guaranteed bit-for-bit reproducible — see [R5](#r5--reproducible-enclave-image)), with the git version embedded via ldflags into `pkg/version/version.go` (`Version`, defaulting to `"dev"`).
