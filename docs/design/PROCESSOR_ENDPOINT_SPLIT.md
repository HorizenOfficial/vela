...
round-robin cursor and the selection view land at **19,475 bytes, 5,101 under the limit**
(`ProcessorEndpointExtension` at 12,172), against the extrapolated ~20,400. `batchStateUpdate()` is
still to account for, and that headroom is what it has to fit in. Same settings as the rest of this
section: hardhat, `cancun`, `runs: 0`.
