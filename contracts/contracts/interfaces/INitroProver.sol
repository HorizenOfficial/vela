...
interface INitroProver {
  ...
  function verifyAttestationStep1(
    ...
    view
    returns (
      bytes[] memory attestation_decoded,
  ...
  function verifyAttestationStep2(
    bytes[] calldata cabundle,
    uint256 index,
    bytes calldata parentPubKey
  ) external view returns (bytes memory pubKey);
  //invoke step 3 after finishing all the 2
  function verifyAttestationStep3(
    bytes[] calldata attestation_decoded,
    bytes calldata certificate,
    bytes calldata parentPubKey
  )
    external
    view
    returns (bytes memory attestationSig, bytes memory pubKey, bytes memory bufBufBuf);
  function verifyAttestationStep4(
